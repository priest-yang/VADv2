...
def load_checkpoint(model,
    ...
    if exclude_keys is not None:
        state_dict = {k: v for k, v in state_dict.items() if not any(k.startswith(exclude_key) for exclude_key in exclude_keys)}
    ...
    try:
        ...
        exclude_keys = re.findall(pattern, str(e))
        state_dict = {k: v for k, v in state_dict.items() if not any(k.startswith(exclude_key) for exclude_key in exclude_keys)}
        missing_keys, unexpected_keys = model.load_state_dict(state_dict, strict=strict)
        missing_keys+=exclude_keys
