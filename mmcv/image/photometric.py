...
def imnormalize_(img, mean, std, to_rgb=True):
    ...
    if isinstance(img, np.ndarray):
        ...
        if to_rgb:
            img = img[..., [2, 1, 0]]
        mean_mask = np.zeros(img.shape, dtype=np.float32)
        mean_mask[:, :, 0] = mean[0]
        mean_mask[:, :, 1] = mean[1]
        mean_mask[:, :, 2] = mean[2]
        mean_mask = torch.from_numpy(mean_mask).float().to(img.device)
        std_mask = np.zeros(img.shape, dtype=np.float32)
        std_mask[:, :, 0] = std[0]
        std_mask[:, :, 1] = std[1]
        std_mask[:, :, 2] = std[2]
        std_mask = torch.from_numpy(std_mask).float().to(img.device)
        img.sub_(mean_mask).div_(std_mask)
    else:
        raise TypeError('img should be numpy array or torch.Tensor')
    return img
