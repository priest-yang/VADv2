...
class LoadRasterMapFromFile:
    ...
    def __call__(self, results):
        ...
        scene_file_name = results["pts_filename"].split("/")[-3]
        frame_idx = results["frame_idx"]    
        results["raster_map"] = self.get_raster_map(scene_file_name, frame_idx)
        return results   
    
    def get_raster_map(self, log_name, index):
        raster_path = osp.join(self.raster_root, log_name, f"{index}.png")
        raster_img = cv2.imread(raster_path).astype(np.float32)
        raster_img = np.ascontiguousarray(raster_img.transpose(2, 0, 1))
        # raster_img = DC(to_tensor(raster_img), stack=True)
        return raster_img


@PIPELINES.register_module()
class LoadRasterGtFromFile:
    def __init__(self,
                 to_float32=False,
                 color_type='color',
                 file_client_args=dict(backend='disk'),
        ...
        self.file_client_args = file_client_args.copy()
        self.file_client = None
        self.raster_gt_root = raster_gt_root
    ...
    def __call__(self, results):
        ...
        scene_file_name = results["pts_filename"].split("/")[-3]
        frame_idx = results["frame_idx"]
        results["raster_gt"] = self.get_raster_map(scene_file_name, frame_idx)
        return results   
    
    def get_raster_map(self, log_name, index):

        raster_gt_path = osp.join(self.raster_gt_root, log_name, f"{index}.png")
        raster_gt = cv2.imread(raster_gt_path, cv2.IMREAD_GRAYSCALE).astype(np.float32)
        raster_gt = np.ascontiguousarray(raster_gt.reshape(1, raster_gt.shape[0], raster_gt.shape[1]))
        raster_gt = DC(to_tensor(raster_gt), stack=True)
        return raster_gt
...
class LoadMultiViewImageFromFiles(object):
    ...
    def __init__(self, to_float32=False, color_type='unchanged', local_path=None):
        self.to_float32 = to_float32
        self.color_type = color_type
    ...
    def __call__(self, results):
        ...
        if 'img' in results:
            ...
            if self.local_path is not None:
                for imgid, name in enumerate(filename):
                    if osp.exists(osp.join(self.local_path, name)):
                        filename[imgid] = osp.join(self.local_path, name)
                    else:
                        print(f"WARNING: {osp.join(self.local_path, name)} not found!")
                # local_img_path = [osp.join(self.local_path, name) for name in filename]
                # filename = local_img_path
