...
class MotionEval(NuScenesEval):
    ...
    def main(self,
        ...
        headers = ["Class Name", "AP", "ATE", "ASE", "AOE", "AVE"]
        print("\t".join(headers))  # 打印表头

        for class_name in class_aps.keys():
            print('\t'.join([
                class_name,
                "%.3f" % class_aps[class_name],
                "%.3f" % class_tps[class_name]['trans_err'],
                "%.3f" % class_tps[class_name]['scale_err'],
                "%.3f" % class_tps[class_name]['orient_err'],
                "%.3f" % class_tps[class_name]['vel_err'],
            ]))
