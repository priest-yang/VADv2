...
class NuScenesDataset(Custom3DDataset):
    ...
    def load_annotations(self, ann_file):
        ...
        """
        data = load(ann_file)
        # data_infos = list(sorted(data['infos'], key=lambda e: e['timestamp'])) should not sort
        data_infos = data if (ann_file.endswith('db') or Path(ann_file).is_dir())else data['infos']
