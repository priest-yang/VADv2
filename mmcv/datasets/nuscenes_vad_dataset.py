...
class VectorizedLocalMap(object):
    ...
    def gen_vectorized_samples(
        self, location, lidar2global_translation, lidar2global_rotation
    ):
        """
    ...
    def get_contour_line(self, patch_box, patch_angle, layer_name, location):
        if (
            layer_name
            not in self.map_explorer[location].map_api.non_geometric_polygon_layers
        ):
        ...
        if layer_name == "drivable_area":
            for record in records:
                polygons = [
                    self.map_explorer[location].map_api.extract_polygon(polygon_token)
    ...
    def get_divider_line(self, patch_box, patch_angle, layer_name, location):
        if (
            layer_name
            not in self.map_explorer[location].map_api.non_geometric_line_layers
        ):
...
class VADCustomNuScenesDataset(NuScenesDataset):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.ann_file_test = self.ann_file
    ...
    def prepare_train_data(self, index):
        ...
        if self.filter_empty_gt and (
            ...
            or (example is None or ~(example["map_gt_labels_3d"]._data != -1).any())
        ):
            return None
        ...
        for i in prev_indexs_list:
            ...
            if (
                ...
                if (
                    ...
                    and (
                        example is None
                        or ~(example["map_gt_labels_3d"]._data != -1).any()
                    )
                ):
                    return None
                frame_idx = input_dict["frame_idx"]
    ...
    def get_ann_info(self, index):
        ...
        if self.with_velocity:
            ...
            try:
                ...
                gt_bboxes_3d = np.full((1, 9), np.nan)
        if self.with_attr:
            ...
            try:
                ...
                attr_labels = np.full((1, 34), np.nan)
                gt_labels_3d = np.full((1, 1), np.nan)
                gt_names_3d = np.full((1, 1), np.nan)
        ...
        if not os.path.exists(self.map_ann_file):
            ...
            for sample_id in range(dataset_length):
                ...
                gt_sample_dict = self.vectormap_pipeline(
                    gt_sample_dict, self.data_infos[sample_id]
                )
        ...
        for metric in map_metrics:
            ...
            for i, name in enumerate(self.MAPCLASSES):
                for j, thr in enumerate(thresholds):
                    if metric == "chamfer":
                        detail["NuscMap_{}/{}_AP_thr_{}".format(metric, name, thr)] = (
                            cls_aps[j][i]
                        )
                    elif metric == "iou":
                        if thr == 0.5 or thr == 0.75:
                            detail[
                                "NuscMap_{}/{}_AP_thr_{}".format(metric, name, thr)
                            ] = cls_aps[j][i]
    ...
    def evaluate(
        ...
        """
        result_metric_names = ["ADE", "FDE", "MR"]
        ...
        for cls in motion_cls_names:
            ...
            if all_metric_dict["cnt_fde_" + cls] != 0:
                ...
                result_dict["FDE_" + cls] = None
                result_dict["MR_" + cls] = None
        ...
        if isinstance(result_files, dict):
            ...
            for name in result_names:
                ...
                ret_dict = self._evaluate_single(
                    result_files[name], metric=metric, map_metric=map_metric
                )
            results_dict.update(ret_dict)
        elif isinstance(result_files, str):
            results_dict = self._evaluate_single(
                result_files, metric=metric, map_metric=map_metric
            )
        ...
        return result_dict, metric_dict, ret_dict
