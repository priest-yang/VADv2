...
class PlanCollisionLoss(nn.Module):
    ...
    def __init__(
        ...
        self.col_idx = torch.tensor([self.class_names.index(cls) for cls in self.col_class])
    ...
    def forward(self,
        ...
        #! updated
        # check not in collision class
        col_idx = self.col_idx.to(agent_max_score_idxs.device)
        col_mask = torch.isin(agent_max_score_idxs, col_idx)
        agent_fut_preds[~col_mask] = 1e6
