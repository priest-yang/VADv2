...
class VADHead(DETRHead):
    ...
    def __init__(
        ...
        *args,
        ...
        loss_raster_seg=dict(type="CrossEntropyLoss", loss_weight=1.0),
        ...
        **kwargs,
        ...
        ## CHANGE
        self.raster_embedding_dim = (
            raster_net.get("quantizer", {}).get("embedding_dim", 256)
            if raster_net is not None
            else 0
        )
        ...
        self.raster_net = build_backbone(raster_net) if raster_net is not None else None
        ...
        self.loss_raster_seg = build_loss(loss_raster_seg)
    ...
    def _init_layers(self):
        ...
        ## CHANGE
        ego_fut_dec_in_dim = ego_fut_dec_in_dim + self.raster_embedding_dim

        for _ in range(self.num_reg_fcs):
            ego_fut_decoder.append(Linear(ego_fut_dec_in_dim, ego_fut_dec_in_dim))
            ego_fut_decoder.append(nn.ReLU())
        ego_fut_decoder.append(
            Linear(ego_fut_dec_in_dim, self.ego_fut_mode * self.fut_ts * 2)
        )
    ...
    def forward(
        ...
        # raster
        loss_vq, raster_logits = 0, None
        if self.raster_embedding_dim > 0:
            raster_embedding, loss_vq, raster_logits = self.raster_net(raster_map)
            raster_embedding = torch.mean(raster_embedding, dim=(2, 3)).unsqueeze(1)
            ego_feats = torch.cat([ego_feats, raster_embedding], dim=-1)

        # Ego prediction
        # TODO change here, add navigation in ego_feats
    ...
    def _get_target_single(
        ...
        # trajs targets
        traj_targets = torch.zeros(
            (num_bboxes, gt_traj_c), dtype=torch.float32, device=bbox_pred.device
        )
        ...
        gt_fut_masks = (
            gt_fut_masks.unsqueeze(-1).repeat(1, 1, 2).view(num_gt_bbox, -1)
        )  # [num_gt_bbox, fut_ts*2]
        ...
        fut_ts_mask = torch.zeros(
            (num_bboxes, self.fut_ts, 2), dtype=torch.float32, device=bbox_pred.device
        )
    ...
    def _map_get_target_single(
        ...
        labels = gt_bboxes.new_full(
            (num_bboxes,), self.map_num_classes, dtype=torch.long
        )
        labels[pos_inds] = gt_labels[sampling_result.pos_assigned_gt_inds]
        label_weights = gt_bboxes.new_ones(num_bboxes)
        # bbox targets
    ...
    def loss_planning(
        ...
        loss_plan_l1_weight = (
            ego_fut_cmd[..., None, None] * ego_fut_masks[:, None, :, None]
        )
        loss_plan_l1_weight = loss_plan_l1_weight.repeat(1, 1, 1, 2)

        loss_plan_l1 = self.loss_plan_reg(
            ego_fut_preds, ego_fut_gt, loss_plan_l1_weight
        )
    ...
    def loss_single(
        ...
        # construct weighted avg_factor to match with the official DETR repo
        traj_cls_avg_factor = (
            num_total_pos * 1.0 + num_total_neg * self.traj_bg_cls_weight
        )
        if self.sync_cls_avg_factor:
    ...
    def get_best_fut_preds(self, traj_preds, traj_targets, gt_fut_masks):
        ...
        best_traj_preds = traj_preds[box_idxs, min_mode_idxs, :, :].reshape(
            -1, self.fut_ts * 2
        )
    ...
    def map_loss_single(
        ...
        if self.map_num_pts_per_vec != self.map_num_pts_per_gt_vec:
            ...
            pts_preds = F.interpolate(
                pts_preds,
                size=(self.map_num_pts_per_gt_vec),
                mode="linear",
                align_corners=True,
            )
    ...
    def loss_raster(self, raster_logits, raster_gt):
        """ "Loss function for raster map.
        Args:
            raster_logits (Tensor): [B, C, H, W]
            raster_gt (Tensor): [B, H, W]
        Returns:
            loss_raster (Tensor): raster map loss.
        """
        loss = dict()
        loss["loss_raster_seg"] = self.loss_raster_seg(
            raster_logits, raster_gt.squeeze(1).long()
        )
        return loss
    ...
    def loss(
        ...
        raster_logits = preds_dicts["raster_logits"]
        ...
        map_all_gt_shifts_pts_list = [
            map_gt_shifts_pts_list for _ in range(num_dec_layers)
        ]
        ...
        if raster_logits is not None:
            loss_raster_dict = self.loss_raster(raster_logits, raster_gt)
            loss_dict.update(loss_raster_dict)
            loss_dict["loss_raster_vq"] = preds_dicts["loss_raster_vq"]
            ego_fut_cmd[0, 0, 0, :] = torch.Tensor([0, 1, 0]).to(
        ...
        agent_fut_preds = all_traj_preds[-1].view(
            batch, num_agent, self.fut_mode, self.fut_ts, 2
        )
        agent_fut_cls_preds = all_traj_cls_scores[-1].view(
            batch, num_agent, self.fut_mode
        )
        loss_plan_input = [
            ego_fut_preds,
            ego_fut_gt,
