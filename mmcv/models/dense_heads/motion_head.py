    ...
    def group_mode_query_pos(self, bbox_results, mode_query_pos):
        ...
        for i in range(batch_size):
            ...
            for j in range(agent_num):
                try:
                    # 尝试访问mode_query_pos的索引
                    grouped_mode_query_pos.append(mode_query_pos[i, j, grouped_label[j]])
                except IndexError:
                    # 如果发生IndexError，打印错误消息并跳过当前迭代
                    grouped_mode_query_pos.append(mode_query_pos[i, j, 0])
                    print(f"IndexError: 索引 {grouped_label[j]} 超出了mode_query_pos的维度范围。")
                    continue
