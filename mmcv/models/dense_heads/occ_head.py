...
class OccHead(BaseModule):
    ...
    def uw_forward_test(
        ...
        if no_query:
            ...
            out_dict['seg_out'] = torch.zeros([1, 5, 1, 200, 200], dtype=torch.long).to(device)  # [1, 5, 1, 200, 200]
            out_dict['ins_seg_out'] = torch.zeros([1, 5, 200, 200], dtype=torch.long).to(device)  # [1, 5, 200, 200]
            return out_dict
