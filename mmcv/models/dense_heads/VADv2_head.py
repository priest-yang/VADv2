...
from mmcv.models.builder import HEADS, build_loss, build_backbone
from mmcv.models.dense_heads import DETRHead
from mmcv.utils import force_fp32, load_checkpoint
...
from mmcv.models.utils.positional_encoding import pos2posemb2d

from mmcv.models.vad_utils.plan_loss import segments_intersect
...
class v116ADTRHead(DETRHead):
    ...
    def __init__(self,
                 *args,
                 ...
                 fut_ts=6,
                 mot_fut_mode=6,
                 ego_fut_mode=3, # [FORWARD, LEFT, RIGHT], temp replace navigation
        ...
        self.plan_anchors = self.plan_anchors[:, :self.fut_ts, :2] # [num_anchors, fut_ts, (x, y)]
    ...
    def _init_layers(self):
        ...
        self.ego_feat_projs = nn.ModuleList(
            [
                ...
                nn.Sequential(
                    nn.Linear(self.ego_fut_mode, self.embed_dims, bias=True),
                    nn.ReLU(inplace=True),
                    nn.Linear(self.embed_dims, self.embed_dims, bias=True),
                    nn.ReLU(inplace=True),
                    nn.Linear(self.embed_dims, self.embed_dims, bias=True),
        ...
        # tl_feats_branch = []
        # tl_feats_branch.append(Linear(2048 * 14 * 24 + 8 * self.embed_dims * 7 * 12, self.embed_dims)) #10 * 13  5 * 7
    ...
    def forward(self,
        ...
        kinodynamic_mask = (torch.norm(self.plan_anchors[:,0,:].to(mot_hs.device), dim=-1) - pred_dis.to(mot_hs.device)).abs() < 100000000000
        ...
        if not isinstance(ego_fut_cmd, torch.Tensor):
            ego_fut_cmd = torch.tensor(ego_fut_cmd, device=ego_map_query.device)
        ego_fut_cmd = ego_fut_cmd.flatten().reshape(ego_fut_cmd.shape[0], 1, self.ego_fut_mode) # [B, 1, num_navi_cmd]
        ego_wp_feat = self.ego_feat_projs[8](ego_fut_cmd)
        ...
        ego_feats = ego_agent_feat + ego_map_feat + ego_wp_feat + ego_status_feat # + ego_cf_feat
                        # 0. * ego_pv_feat
                        # 0. * ego_cmdid_feat + \
    ...
    def get_bboxes(self, preds_dicts, img_metas, rescale=False):
        ...
        det_preds_dicts = self.bbox_coder.decode(preds_dicts, cls_key='all_cls_scores', bbox_key='all_bbox_preds', traj_key='all_mot_preds')
...
class PlanningMetric():
    ...
    def evaluate_coll(
        ...
        for i in range(B):
            ...
            m2 = torch.ones_like(gt_box_coll).to(ti.device)
