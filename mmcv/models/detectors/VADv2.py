...
class VADv2(MVXTwoStageDetector):
    """VADv2 model.
    """
    def __init__(self,
                 use_grid_mask=False,
    ...
    def forward_pts_train(self,
        ...
        # if have raster map, set the command to forward (bypass)
        if self.pts_bbox_head.raster_net is not None:
    ...
    def forward(self, return_loss=True, rescale=False, **kwargs):
        ...
        if return_loss:
            ...
            outputs = dict(
                loss=loss, log_vars=log_vars, num_samples=len(kwargs['img_metas']))
            return outputs
        else:
            outputs = self.forward_test(**kwargs,rescale=rescale)            
            return outputs
    ...
    def forward_test(
        ...
        **kwargs
        ...
        # cur_pos = tmp_pos#copy.deepcopy(img_metas[0][0]['can_bus'][:3])
        # cur_angle = tmp_angle#copy.deepcopy(img_metas[0][0]['can_bus'][-1])
        # cur_posture={
        #     "pos":cur_pos,
        #     "angle":cur_angle
        #     }
        # bbox_results[0]["pts_bbox"].update(cur_posture)
        return {
            'bbox_results': bbox_results,
        }

    def uw_forward_inference(
            self,
            img_metas,
            img=None,
            ego_his_trajs=None,
            ...
            **kwargs
            ...
            for var, name in [(img_metas, 'img_metas')]:
                if not isinstance(var, list):
                    raise TypeError('{} must be a list, but got {}'.format(
                        name, type(var)))
            img = [img] if img is None else img
            img_metas[0][0]['box_type_3d'] = LiDARInstance3DBoxes
            ...
            new_prev_bev, bbox_results = self.uw_simple_test(
                img_metas=img_metas[0],
                img=img[0],
                prev_bev=self.prev_frame_info['prev_bev'],
                ego_his_trajs=ego_his_trajs[0],
            ...
            ego_pos = tmp_pos#copy.deepcopy(img_metas[0][0]['can_bus'][:3])
            ego_angle = tmp_angle#copy.deepcopy(img_metas[0][0]['can_bus'][-1])
            ego_posture={
                "pos":ego_pos,
                "angle":ego_angle
                }
            bbox_results[0]["pts_bbox"].update(ego_posture)
            return bbox_results
    ...
    def uw_simple_test(
        ...
        **kwargs
        ...
        img_feats = self.extract_feat(img=img, img_metas=img_metas)
        bbox_list = [dict() for i in range(len(img_metas))]
        new_prev_bev, bbox_pts, metric_dict = self.uw_simple_test_pts(
            img_feats,
            img_metas,
            prev_bev,
            fut_valid_flag=fut_valid_flag,
    ...
    def simple_test_pts(
        ...
        bbox_results = []
        for i, (bboxes, scores, labels, trajs, trajs_cls, map_bboxes, \
        ...
        with torch.no_grad():
            ...
            ego_fut_pred = bbox_result['ego_fut_preds']

            # ego_fut_pred = ego_fut_pred.cumsum(dim=-2)
            ego_fut_trajs = ego_fut_trajs.reshape(-1, 2).cumsum(dim=-2)
    ...
    def uw_simple_test_pts(
        ...
        with torch.no_grad():
            ...
            # filter pred bbox by score_threshold
            mask = bbox_result['scores_3d'] > score_threshold
            # mask = torch.ones(bbox_result['scores_3d'].shape, dtype=torch.bool)
            bbox_result['boxes_3d'] = bbox_result['boxes_3d'][mask]
            bbox_result['scores_3d'] = bbox_result['scores_3d'][mask]
            bbox_result['labels_3d'] = bbox_result['labels_3d'][mask]
            bbox_result['trajs_3d'] = bbox_result['trajs_3d'][mask]
            ...
            metric_dict = {}
            # metric_dict_planner_stp3 = self.uw_compute_planner_metric_stp3(
            #     pred_ego_fut_trajs = ego_fut_pred[None],
            #     fut_valid_flag = fut_valid_flag
            # )
            # metric_dict.update(metric_dict_planner_stp3)
    ...
    def compute_motion_metric_vip3d(
        ...
        veh_list = [12]
        ignore_list = [
            'vehicle', 'truck', 'construction_vehicle', 'bus', 'trailer', 'barrier',
    ...
    ### same planning metric as stp3
    def uw_compute_planner_metric_stp3(
        self,
        pred_ego_fut_trajs,
        fut_valid_flag
    ):
